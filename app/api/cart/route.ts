...
export async function GET(request: NextRequest) {
    try {
        ...
        const totalItems = cartItems.reduce((sum: number, item: typeof cartItems[0]) => sum + item.quantity, 0)
        const totalPrice = cartItems.reduce(
            (sum: number, item: typeof cartItems[0]) => sum + Number(item.product.price) * item.quantity,
            0
        )
